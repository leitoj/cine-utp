...
public class Sales {
    private int popcorn = 0, refresco = 0, agua = 0, soda = 0, hotdog = 0, chocolate = 0, totalAmount = 0,
            totalSell = 0;

    private ArrayList<String> invoices = new ArrayList<String>();

    public void insertSale(ArrayList<Product> sales) {
        for (int i = 0; i < sales.size(); i++) {
            String invoice = "\t\t" + i + ". \t " + sales.get(i).getName() + "(" + sales.get(i).getAmount()
                    + ")  |  Pago total... $ "
                    + (sales.get(i).getAmount() * sales.get(i).getPrice()) * 1.07;
